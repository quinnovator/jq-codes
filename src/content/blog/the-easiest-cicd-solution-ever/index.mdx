---
title: The Easiest CI/CD Solution Ever
description: I recently dug into SST v3 as an option for managing my AWS resources as code. It's got a ton of potential.
date: 2024-09-29
tags: ['CI/CD', 'sst', 'devops', 'IaC']
image: './thumb.png'
authors: ['Jack']
---
...
One thing I've learned about myself over the years is that if I want to make progress on something, I need to make it as easy as possible to iterate. It's happened too many times that I have spun up a new project, and then gotten lost in the weeds trying to set up preview environments on PRs, or auto-deploy to my Kubernetes cluster.

The truth is that while it might a satisfying project to set up a complex pipeline on your own hardware, the time spent configuring this type of setup and overheard required to keep it running is just too much.

So, where does that leave us?

Well, there are a lot of PaaS offerings out there like Vercel, Netlify, or Heroku. Those offerings are a bit too limited for me. Practically I may find a platform like one of these sufficient for my needs. Especially if I used a more generalized serverless compute PaaS service like Fly.io. The thing is, I have always seen services like these as just wrappers on AWS. I also dislike that I don't have centralized billing.

So, I recently took a look at SST and its recent v3 release. SST is a TypeScript-first framework that allows us to define infrastructure as code (IaC) in a single config file that make up an application.

My initial thoughts - nothing beats SST in terms of time to production. I deployed an API using API Gateway with CORS enabled in about 5 minutes flat.
